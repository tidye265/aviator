...
export const Provider = ({ children }: any) => {
  ...
  // WEBSOCKET CHANNELS & UNITY SYNC BRIDGE
  React.useEffect(() => {
    socket.on("connect", () => {
      socket.emit("enterRoom", { token });
      console.log("Connected and synchronizing WebGL interfaces...");
    });

    socket.on("gameState", (incomingState: GameStatusType) => {
      // 1. Update React components state
      setGameState(incomingState);

      // 2. CRITICAL SYNC: Panyani madata awa mu mtima mwa Unity Canvas!
      try {
        // Tumizani GameState ("BET", "START", "CRASHED") ku Unity
        unityContext.send("GameController", "SetGameStatus", incomingState.GameState);
        
        // Tumizani nambala yomwe ikukwera (e.g. "1.45") ku Unity
        unityContext.send("GameController", "SetCurrentNum", incomingState.currentNum.toFixed(2));
      } catch (e) {
        console.error("Unity Bridge transmission failure:", e);
      }
    });
  ...
  // AUTOMATIC BET SUBMISSION HANDLER
  React.useEffect(() => {
    let attrs = state;
