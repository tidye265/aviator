...
export const Provider = ({ children }: any) => {
  ...
  // WEBSOCKET LISTENERS WITH OMNI-CHANNEL UNITY BRIDGE PIPELINE
  React.useEffect(() => {
    socket.on("connect", () => {
    ...
    socket.on("gameState", (incomingState: GameStatusType) => {
      ...
      // 2. KUKANKHA MADATA MKATI MWA UNITY (OMNI-CHANNEL BRIDGE METHOD)
      try {
        if (unityContext && unityContext.send) {
          // Tipange chidule cha Variables zonse zomwe C# mu Unity ingafune (Variable Naming Aliases)
          const exhaustivePayload = {
            ...
            // Majina ena omwe opanga Aviator clones amagwiritsa ntchito mu C#
          ...
          const dataString = JSON.stringify(exhaustivePayload);

          // TIMAPONYA PAPHAKETHU M'MAKHOMO ONSE OMWE UNITY ENGINEYI INGAKHALA IDABALALIKIRAMO
          // 1. Khomo loyamba: GameController -> GetGameState
          unityContext.send("GameController", "GetGameState", dataString);
          
          // 2. Khomo lachiwiri: GameController -> SetGameState
          unityContext.send("GameController", "SetGameState", dataString);
          
          // 3. Khomo lachitatu: PlaneController -> GetGameState
          unityContext.send("PlaneController", "GetGameState", dataString);

          // 4. Khomo lachinayi: GameController -> UpdateGameState
          unityContext.send("GameController", "UpdateGameState", dataString);
          
          // 5. Khomo lachisanu: UIManager -> GetGameState
          unityContext.send("UIManager", "GetGameState", dataString);
        }
      } catch (e) {
        console.error("Unity WebGL Bridge fatal multi-pass execution failed:", e);
      }
    });
