...
export const Provider = ({ children }: any) => {
  ...
  // UNITY CONTROLLER LIFECYCLE LISTENERS
  ...
  // WEBSOCKET LISTENERS WITH HIGH-SPEED UNITY BRIDGE PIPELINE
  React.useEffect(() => {
    socket.on("connect", () => {
    ...
    socket.on("gameState", (incomingState: GameStatusType) => {
      // 1. Kusintha madata a pa React Screen UI
      setGameState(incomingState);

      // KUKHWIMBITSA TSOPANO: NKHONDO YA BRIDGE YA UNITY CONTEXT
      try {
        // Pa react-unity-webgl ya mtundu wa UnityContext, njira yotumizira ndi "unityInstance.SendMessage" 
        // kapena kugwiritsa ntchito internal communication window yomwe ili mu unityContext.
        const unityInstance = (unityContext as any).unityInstance;
        
        if (unityInstance && unityInstance.SendMessage) {
          const rawMultiplier = incomingState.currentNum ? incomingState.currentNum.toFixed(2) : "1.00";
          
          const payload = JSON.stringify({
            currentNum: incomingState.currentNum,
            currentSecondNum: incomingState.currentSecondNum,
            multiplier: incomingState.currentNum,
            rate: incomingState.currentNum,
            GameState: incomingState.GameState,
            state: incomingState.GameState,
            status: incomingState.GameState,
            gameStatus: incomingState.GameState,
            time: incomingState.time
          });

          // Mayina a ma GameObject omwe alimo m'masewerawa ndendende
          ...
          targetGameObjects.forEach((targetObject) => {
            try {
              // Kuyesa kutumiza string ya nambala (Mwachitsanzo: "2.34")
              unityInstance.SendMessage(targetObject, "GetGameState", rawMultiplier);
              unityInstance.SendMessage(targetObject, "UpdateMultiplier", rawMultiplier);
              unityInstance.SendMessage(targetObject, "SetMultiplier", rawMultiplier);
              unityInstance.SendMessage(targetObject, "SetCurrentNum", rawMultiplier);
              unityInstance.SendMessage(targetObject, "SetCurrentMulti", rawMultiplier);
              unityInstance.SendMessage(targetObject, "OnGameStateUpdate", rawMultiplier);

              // Kuyesa kutumiza JSON string data package
              unityInstance.SendMessage(targetObject, "GetGameState", payload);
              unityInstance.SendMessage(targetObject, "SetGameState", payload);
              unityInstance.SendMessage(targetObject, "UpdateGameState", payload);
            } catch (innerError) {
              // Kupewa kulephera kwa GameObject imodzi kulepheretsa zina zotsatira
            }
          });
        }
      } catch (e) {
        console.error("Unity WebGL Bridge Error:", e);
      }
    });
  ...
  // PLACE AUTOMATIC OR MANUAL BET ACTIONS INTERCEPTOR
  React.useEffect(() => {
    let attrs = state;
  ...
  return (
    <Context.Provider
      value={{
        state: state, ...betLimit, ...userBetState, ...unity, ...gameState, currentTarget, rechargeState,
