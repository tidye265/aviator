...
export const Provider = ({ children }: any) => {
  ...
  // WEBSOCKET LISTENERS WITH CHITETEZO CHA MEMORI (ANTI-CRASH)
  React.useEffect(() => {
    socket.on("connect", () => {
    ...
    socket.on("gameState", (incomingState: GameStatusType) => {
      // 1. Kusintha madata pa React Context UI
      setGameState(incomingState);

      // 2. KUTUMIZA MA LIVE ODDS POPANDA KUGWETSA WEBGL
      try {
        const unityInstance = (unityContext as any).unityInstance;
        
        // Ngati masewera asanamalize kutsegula, musatumize madata kaye
        if (unityInstance && unityInstance.SendMessage) {
          const rawMultiplier = incomingState.currentNum ? incomingState.currentNum.toFixed(2) : "1.00";

          // NTCHITO CHACHIKULU: Tumizani ku "WebGLBridge" yomwe ilidi m'masewera anu!
          // Chotsani mayina onse achinyengo omwe akubweretsa "object not found"
          unityInstance.SendMessage("WebGLBridge", "GetGameState", rawMultiplier);
          
          // Ngati script yanu ikusowa "WebGLBridge", itumizeni pa object ya m'tsogolo yomwe idalipo mwa apo ndi apo:
          unityInstance.SendMessage("GameController", "GetGameState", rawMultiplier);
        }
      } catch (e) {
        console.error("Unity WebGL Communication Error prevented crash:", e);
      }
    });
