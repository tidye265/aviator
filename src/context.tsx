...
export const Provider = ({ children }: any) => {
  ...
  React.useEffect(() => {
    ...
    socket.on("gameState", (incomingState: GameStatusType) => {
      ...
      // 2. KUKANKHA MADATA MKATI MWA UNITY (PRO LEVEL REAL-TIME RESOLUTION)
      // Timatumiza Chiphakethu chonse cha JSON pa 100ms kudzera mu function imodzi
      try {
        if (unityContext && unityContext.send) {
          unityContext.send("GameController", "GetGameState", JSON.stringify(incomingState));
        }
      } catch (e) {
        console.error("Unity WebGL Bridge failed to transfer live state package:", e);
      }
    });
